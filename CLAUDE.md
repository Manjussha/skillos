...
Two post-v0.1 additions ride on the existing protocol (no new ServerMessage types): a **runtime LLM picker** — `/provider` (local-only; lists all registry providers via `providerInfo()` with active `*` / keyed `✓` markers, switches the active provider or connects an API key via the generalized `applyProviderChoice(providerId, apiKey)`, which writes the provider's `envKey` + `SKILLOS_PROVIDER` to `.env` and `process.env` live) and `/models` (interactive numbered selection of the **active provider's** real models via `modelsFor()`); both use a `session.picker` state handled before command parsing (a `/command` cancels an active picker). And **cross-session memory** — `apps/server/src/memory/memory.ts` compresses a session into `storage/memory/<userId>.md` on disconnect (model summary if a provider is set, else a deterministic offline digest) and recalls it on the next connection, injecting a bounded preamble into the system prompt. Verifier: `scripts/verify-memory.mjs`.
...
- `providers/provider.ts` — a **provider registry** with runtime selection. `resolveProvider(logical, mode)` picks the **active** provider (see below) and maps the logical name (or a concrete model id) + the session's **mode** to a concrete model, then streams text. Eight `ProviderKind`s: direct **openai / anthropic / google / groq / deepseek** (each via its `@ai-sdk/*` factory, key read from `process.env` at call time), **openrouter** (via `@ai-sdk/openai` pointed at OpenRouter's OpenAI-compatible endpoint, still honoring `mode` best/cheap tiers), **ollama** (direct `fetch` to its NDJSON streaming API — no SDK), and a **mock** provider used when nothing is configured so the loop is demoable offline. Each registry entry declares `id`, `label`, `envKey`, `needsKey`, selectable `models`, a `logical` map, and `defaultModel`. **Active-provider selection:** `activeProvider()` reads `SKILLOS_PROVIDER`; if unset or its key is missing it auto-detects (first keyed provider in priority order — OpenRouter first to preserve prior behavior — else Ollama if `OLLAMA_BASE_URL`, else mock). `local` mode still forces Ollama when configured. Helpers `providerInfo()` (for `/provider`) and `modelsFor(id)` (for `/models`) introspect the registry live. **Adding a provider** = one registry entry + a factory case in the `streamCompletion` switch.
